...
def plot_2D_heatmap(model, title="Set Title", scale=10.0, type='first', show_data=False):
    
    # scale of the heat maps
    X1 = np.arange(-scale, scale+0.01, scale/100.0)
    ...
    heatmap_preds_softmax = np.array([np.exp(x)/sum(np.exp(x)) for x in heatmap_preds])
    ...
    if show_data!=False:
        x,y = zip(*show_data.dataset.tensors[0].detach().cpu().numpy())
